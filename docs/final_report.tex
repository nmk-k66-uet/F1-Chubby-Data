...
\begin{enumerate}
    ...
    \item \textbf{Xử lý phân tán với Spark}: Huấn luyện mô hình ML trên dữ liệu lịch sử 2022-2025 với two-job feature extraction và training pipeline trên Google Dataproc.
...
Streamlit -> core/data_loader.py
  ...
  -> Check GCS bucket (download if found)
  -> Fallback to FastF1 API
  -> Upload to GCS (only if fetched from API)
\end{lstlisting}
...
\begin{itemize}
    \item Download từ GCS to local cache khi local miss.
    \item Pre-populated với historical data (2018-2026).
    \item Các session được cache sẵn không cần download từ API.
\end{itemize}
...
\begin{itemize}
    ...
    \item Result được upload lên GCS async (background thread) cho lần sau.
\end{itemize}
...
\subsection{Two-Job Training Architecture}

Pipeline được tách thành 2 independent Spark jobs để improve debugging và iteration speed:

\textbf{Job 1 - Feature Extraction} (\texttt{feature\_extraction\_job.py}):
\begin{itemize}
    \item Reads FastF1 cache from GCS (zero API calls)
    \item Processes 2022-2025 seasons in parallel (4 partitions)
    \item Extracts features using \texttt{mapInPandas}
    \item Outputs CSV to \texttt{gs://f1chubby-raw/processed\_features/}
\end{itemize}

\textbf{Job 2 - Model Training} (\texttt{model\_training\_job.py}):
\begin{itemize}
    \item Reads feature CSVs from GCS
    \item Trains 3 Random Forest models with GridSearchCV/RandomizedSearchCV
    \item Validates sklearn version compatibility
    \item Uploads \texttt{.pkl} files to \texttt{gs://f1chubby-model/}
\end{itemize}

Benefits:
\begin{itemize}
    \item Re-train models without re-extracting features (saves 10-15 min)
    \item Inspect/validate feature CSVs between jobs
    \item Clear separation: data engineering vs machine learning
\end{itemize}

\textbf{Job 1 Parallel Processing}:
\begin{lstlisting}[language=Python]
...
df_pre_race = df_years.mapInPandas(
    extract_pre_race_features, schema)
df_in_race = df_years.mapInPandas(
    extract_in_race_features, schema)
\end{lstlisting}

\textbf{Pre-Race Features}:
...
\subsection{Model Training Workflow (Job 2)}

Job 2 loads feature CSVs và trains 3 Random Forest classifiers:

\textbf{Pre-Race Podium Model - GridSearchCV}:
\begin{lstlisting}[language=Python]
base_pre = RandomForestClassifier(
    random_state=42,
...
joblib.dump(grid_pre.best_estimator_,
  'podium_model.pkl')
\end{lstlisting}

GridSearchCV với 5-fold CV tìm best hyperparameters. \texttt{class\_weight='balanced'} xử lý imbalance (only 3/20 drivers podium mỗi race).

\textbf{In-Race Models - RandomizedSearchCV}:
\begin{lstlisting}[language=Python]
...
In-race models train trên toàn bộ laps dataset (~30,000 samples từ 4 seasons × ~20 races × ~20 drivers × ~50 laps). RandomizedSearchCV faster than GridSearchCV for large parameter space.

\textbf{Model Validation}:
\begin{lstlisting}[language=Python]
# Test reload + predict to catch sklearn
# version mismatch before deployment
for model_path in model_files:
    model = joblib.load(model_path)
    X_test = pd.DataFrame([[0.5]*n_features])
    prediction = model.predict_proba(X_test)
    # Raises error if incompatible
\end{lstlisting}

Validation prevents deploying models trained with different sklearn version than serving API.

\subsection{Spark Execution Plan}

\textbf{Job 1 Configuration}:
\begin{lstlisting}[language=Python]
spark = SparkSession.builder \
    .appName("F1_Feature_Extraction") \
    .getOrCreate()

# Distribute 4 years across workers
# With 2 workers: each processes 2 years
years = [(2022,), (2023,), (2024,), (2025,)]
df_years = spark.createDataFrame(
    years, ["Year"]).repartition(4)

# mapInPandas runs on each partition
df_pre = df_years.mapInPandas(
    extract_pre_race_features, schema)
\end{lstlisting}

Feature extraction uses \texttt{mapInPandas} cho parallel processing. Mỗi worker downloads cache cho assigned years, extracts features, returns Pandas DataFrame. Spark combines results và writes to GCS.

\textbf{Job 2 Configuration}:
\begin{lstlisting}[language=Python]
spark = SparkSession.builder \
    .appName("F1_Model_Training") \
    .getOrCreate()

# Load features from GCS
pre_race_df = spark.read.csv(
    "gs://.../pre_race_features",
    header=True, inferSchema=True
).toPandas()
\end{lstlisting}

Training không distribute (scikit-learn chạy trên driver node), nhưng feature loading benefit từ Spark's parallel read. Với 2 workers × 2 cores, Job 1 hoàn thành trong ~12 phút cho 4 years data, Job 2 trong ~5 phút.
...
\begin{itemize}
    \item Job 1 (Feature Extraction): 12 minutes (2022-2025 data, 4 years)
    \item Job 2 (Model Training): 5 minutes (3 models with hyperparameter tuning)
    \item Total pipeline time: ~17 minutes (Job 1 + Job 2)
